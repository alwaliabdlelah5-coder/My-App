...
export default function InventoryPage() {
  const { inventory: liveInventory, loading, addItem } = useInventory();
  const [activeTab, setActiveTab] = useState<'all' | 'supplies' | 'equipment'>('all');
  const [searchQuery, setSearchQuery] = useState('');

  const displayInventory = loading ? inventoryData : (liveInventory.length > 0 ? liveInventory : inventoryData);
