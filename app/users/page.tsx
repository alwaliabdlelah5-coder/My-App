...
export default function UsersPage() {
  ...
  const [newUser, setNewUser] = useState({ name: '', email: '', role: '', status: 'نشط' as const });
              ...
              >
                 ...
                 <div className="p-10 space-y-8">
                    <div className="grid grid-cols-2 gap-8">
                       <FormInput 
                         label="Full Medical Name" icon={Users} placeholder="د. خالد محمد..." 
                         value={newUser.name} onChange={(e) => setNewUser({...newUser, name: e.target.value})}
                       />
                       <FormInput 
                         label="Email Address" icon={Mail} placeholder="name@medical.ye" 
                         value={newUser.email} onChange={(e) => setNewUser({...newUser, email: e.target.value})}
                       />
                    </div>
                    <div className="grid grid-cols-2 gap-8">
                       <FormInput 
                         label="Assigned Role" icon={ShieldCheck} placeholder="Select Permission Level" 
                         value={newUser.role} onChange={(e) => setNewUser({...newUser, role: e.target.value})}
                       />
                       <FormInput 
                         label="Security Level" icon={Lock} placeholder="Alpha / Beta / Gamma" 
                         value="" onChange={() => {}}
                       />
                    </div>
                    <button 
...
function FormInput({ label, icon: Icon, placeholder, value, onChange }: { label: string, icon: any, placeholder: string, value: string, onChange: (e: any) => void }) {
  return (
