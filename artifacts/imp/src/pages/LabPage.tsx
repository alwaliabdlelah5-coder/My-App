...
import { FlaskConical, Search, Plus, Clock, Beaker, Dna, Microscope, FileText, ChevronLeft, TrendingUp } from 'lucide-react';
...
function LabParameterInput({ label, unit, range, value, onChange }: { label: string, unit: string, range: string, value: string, onChange: (val: string) => void }) {
  ...
  return (
    <div className="flex items-center gap-4 group">
      <div className="w-32"><p className="font-black text-gray-900 text-base italic leading-none">{label}</p><p className="text-[10px] text-gray-400 font-black uppercase italic mt-1 tracking-tighter">{unit}</p></div>
...
export default function LabPage() {
  ...
  const currentTest = testDefinitions[selectedOrder?.testId || 'heme'] || testDefinitions['heme'];
        ...
        <div className="flex items-center gap-6 overflow-x-auto py-2">
          {testCategories.map((cat, i) => (
            ...
            >
              ...
              <div><h3 className="font-bold text-gray-900 whitespace-nowrap">{cat.name}</h3><p className="text-xs text-gray-400 font-bold">{cat.count} فحص قيد العمل</p></div>
            </motion.div>
          ))}
        </div>
        ...
        <div className="bg-white rounded-[2.5rem] border shadow-sm flex flex-col overflow-hidden min-h-[600px]">
          <div className="p-8 border-b bg-gray-50/50 flex justify-between items-center">
            ...
            <div className="flex gap-2">
              <button onClick={() => setActiveTab('pending')} className={cn("px-6 py-2 bg-white border rounded-xl font-bold text-sm transition-all shadow-sm", activeTab === 'pending' ? "text-primary border-primary" : "text-gray-500")}>طلبات نشطة</button>
              <button onClick={() => setActiveTab('history')} className={cn("px-6 py-2 bg-white border rounded-xl font-bold text-sm transition-all shadow-sm", activeTab === 'history' ? "text-primary border-primary" : "text-gray-500")}>مكتملة</button>
            </div>
          </div>
          ...
          <div className="flex-1 p-8 grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
            <div className="space-y-4">
              {pendingOrders.map((order, i) => (
                <motion.div key={order.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }}
                  ...
                  className={cn("p-6 bg-gray-50 border rounded-3xl group hover:border-primary/20 transition-all flex items-center justify-between cursor-pointer",
                    selectedOrder?.id === order.id && "border-primary bg-primary/5"
                  )}
                >
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-white rounded-2xl border flex items-center justify-center text-primary shadow-sm group-hover:scale-110 transition-all"><FileText className="w-6 h-6" /></div>
                    <div>
            ...
            <div className="bg-white rounded-[2rem] border-2 border-dashed border-gray-200 p-8 space-y-8 shadow-inner">
              ...
              <div className="pt-6 border-t space-y-4">
                ...
                <textarea className="w-full bg-gray-50 border rounded-2xl p-4 outline-none focus:ring-4 focus:ring-primary/10 font-bold text-sm h-24 placeholder:italic" placeholder="أضف ملاحظات فني المختبر هنا..." />
              </div>
              <button onClick={() => alert('تم حفظ النتائج بنجاح وإرسال التقرير للأرشفة')}
