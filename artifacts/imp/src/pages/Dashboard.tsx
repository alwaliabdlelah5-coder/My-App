...
import { 
  Users, Calendar, TrendingUp, Activity, ArrowUpRight, Clock,
  ArrowDownRight, Plus, ClipboardList, Stethoscope, Package, CreditCard, ChevronLeft
} from 'lucide-react';
...
const growthData = [
  { day: 'Sat', patients: 12 }, { day: 'Sun', patients: 18 }, { day: 'Mon', patients: 15 },
  { day: 'Tue', patients: 22 }, { day: 'Wed', patients: 30 }, { day: 'Thu', patients: 25 }, { day: 'Fri', patients: 10 },
];

export default function Dashboard() {
  const { patients: livePatients } = usePatients();
  const { appointments: liveApps } = useAppointments(new Date().toISOString().split('T')[0]);
  ...
  const stats = [
    { name: 'إجمالي المرضى', value: livePatients.length > 0 ? livePatients.length.toString() : '42', trend: '+12%', trendUp: true, icon: Users, color: 'blue' },
    { name: 'مواعيد اليوم', value: liveApps.length > 0 ? liveApps.length.toString() : '24', trend: '3 غياب', trendUp: false, icon: Calendar, color: 'emerald' },
    { name: 'الإيرادات اليومية', value: `${finStats.income.toLocaleString()} ر.ي`, trend: '+8%', trendUp: true, icon: TrendingUp, color: 'indigo' },
    { name: 'حالات في الانتظار', value: liveQueue.filter(q => q.status === 'waiting').length.toString() || '0', trend: 'متوسط 15 د', trendUp: true, icon: Clock, color: 'orange' },
  ];

  const upcomingAppointments = liveApps.length > 0 ? liveApps.slice(0, 3).map(app => ({
    id: app.id, name: app.patientName, time: app.startTime, type: app.type, doctor: app.doctorName,
      ...
      <div className="space-y-12">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
            <h1 className="text-4xl font-black text-gray-900 italic tracking-tighter uppercase font-mono">Command Center</h1>
            <p className="text-gray-500 mt-1 font-bold italic uppercase text-[10px] tracking-[0.2em]">Welcome back, Dr. Ahmed • Hospital Performance Overview</p>
          </motion.div>
          <div className="flex gap-4">
            <button className="bg-white border-2 border-gray-100 text-gray-900 px-8 py-3 rounded-2xl font-black italic tracking-tighter shadow-sm hover:border-primary transition-all">View Schedule</button>
            ...
            >
              <div className="flex items-start justify-between relative z-10">
                <div className={cn("w-14 h-14 rounded-2xl flex items-center justify-center transition-all group-hover:scale-110",
                  ...
                  stat.color === 'indigo' ? "bg-indigo-50 text-indigo-600" : "bg-orange-50 text-orange-600"
                )}>
                  <stat.icon className="w-7 h-7" />
                </div>
                <div className={cn("flex items-center gap-1 text-[10px] font-black italic uppercase px-3 py-1 rounded-full",
          ...
          <div className="lg:col-span-2 space-y-8">
                  ...
                  <BarChart data={growthData}>
                    ...
                    <Tooltip cursor={{ fill: 'transparent' }} content={({ active, payload }) => {
                      if (active && payload && payload.length) {
                        return <div className="bg-gray-900 text-white p-4 rounded-2xl shadow-xl font-black italic text-xs tracking-widest uppercase">{payload[0].value} PATIENTS</div>;
                      }
                      return null;
                    }} />
            ...
            <div className="bg-white rounded-3xl border shadow-sm overflow-hidden">
              <div className="p-6 border-b flex items-center justify-between">
                <h2 className="font-bold text-xl flex items-center gap-2"><Calendar className="w-5 h-5 text-primary" />المواعيد القادمة (اليوم)</h2>
                ...
                <table className="w-full text-right">
                  <thead>
                    <tr className="bg-gray-50 text-gray-400 text-xs font-bold uppercase tracking-wider">
                      ...
                      <th className="px-6 py-4">نوع الزيارة</th>
                    ...
                    {upcomingAppointments.map((app) => (
                      <tr key={app.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-6 py-4 text-center">
                          <div className={cn("mx-auto w-fit px-3 py-1 rounded-full text-xs font-bold",
            ...
            <div className="bg-white rounded-3xl border shadow-sm overflow-hidden">
              <div className="p-6 border-b flex items-center justify-between bg-orange-50/50">
                <h2 className="font-bold flex items-center gap-2 text-xs uppercase tracking-widest italic text-orange-600">
                  <ClipboardList className="w-4 h-4" />قائمة الانتظار
                </h2>
                <span className="bg-orange-500 text-white text-[8px] px-2 py-0.5 rounded-full font-black uppercase">{liveQueue.filter(q => q.status === 'waiting').length} CASES</span>
              </div>
                ...
                <div className="pt-4 border-t border-dashed">
                  <Link href="/queue">
                    <button className="w-full py-3 text-[10px] font-black text-gray-400 uppercase tracking-widest hover:text-primary transition-colors flex items-center justify-center gap-2 group">
                      MANAGE FULL QUEUE <ChevronLeft className="w-3 h-3 group-hover:-translate-x-1 transition-transform" />
                    </button>
                  </Link>
                </div>
              </div>
            ...
            <div className="bg-gray-900 rounded-[2.5rem] p-8 text-white space-y-4 relative overflow-hidden group">
              <div className="relative z-10">
                <div className="flex items-center gap-3">
                  <div className="p-3 bg-white/5 rounded-2xl"><Activity className="w-6 h-6 text-primary" /></div>
                ...
                <p className="text-sm text-white/40 leading-relaxed mt-6 font-bold uppercase tracking-tight italic">
                  {criticalStock ? <>Critical shortage detected for <span className="text-primary">"{criticalStock.name}"</span>. Only {criticalStock.stock} units remaining.</> : <>Inventory levels are within safe parameters. No critical shortages detected.</>}
                </p>
...
function QuickAction({ icon: Icon, label, href, color }: { icon: any, label: string, href: string, color: string }) {
  return (
    <Link href={href}>
      <motion.button whileHover={{ scale: 1.05, y: -5 }} whileTap={{ scale: 0.95 }}
        className="w-full bg-white p-6 rounded-[2.5rem] shadow-sm flex flex-col items-center gap-4 transition-all hover:shadow-xl hover:shadow-primary/5 group border border-transparent hover:border-primary/10"
      >
