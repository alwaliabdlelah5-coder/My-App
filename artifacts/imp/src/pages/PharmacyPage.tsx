...
import { Tablets, Search, Plus, AlertCircle, Package, TrendingUp, ArrowRight, Filter, MoreVertical, Clock } from 'lucide-react';
...
export default function PharmacyPage() {
  ...
  const [dispenseAmount] = useState(1);
  ...
  const handleConfirmDispense = async () => {
    if (!selectedDrug || !dispenseTarget) { alert('الرجاء اختيار الدواء والمريض'); return; }
    if (selectedDrug.stock < dispenseAmount) { alert('المخزون غير كافٍ'); return; }
    ...
    await addTransaction({ type: 'income', category: 'الصيدلية', amount: priceVal * dispenseAmount, description: `صرف ${selectedDrug.name} لـ ${dispenseTarget}`, method: 'cash' });
    alert('تم صرف الدواء بنجاح وتسجيل العملية المالية');
    setActiveTab('inventory'); setSelectedDrug(null); setDispenseTarget('');
  };
    ...
    <Sidebar>
      <div className="space-y-8">
        <div className="flex items-center justify-between">
          ...
          <button className="bg-primary text-white px-6 py-3 rounded-2xl font-bold flex items-center gap-2 hover:bg-primary/95 transition-all shadow-lg shadow-primary/25"><Plus className="w-5 h-5" />إضافة صنف جديد</button>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-6 items-center justify-between">
          ...
          <div className="relative w-full md:w-96">
            ...
            <input type="text" placeholder="ابحث عن دواء (اسم علمي أو تجاري)..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
          ...
          <AnimatePresence mode="wait">
            {activeTab === 'inventory' && (
              <motion.div key="inventory" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                <div className="p-8 border-b bg-gray-50/50 flex justify-between gap-6">
                  <div className="relative flex-1 max-w-xl">
                    <Search className="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
                    <input type="text" placeholder="بحث باسم الدواء العلمي أو التجاري..." className="w-full pr-12 pl-4 py-4 bg-white border-2 border-gray-100 rounded-[1.5rem] focus:border-primary outline-none font-bold text-[16px] transition-all" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                  </div>
                  <div className="flex gap-3">
                    <button className="flex items-center gap-2 px-6 py-3 bg-white border-2 border-gray-100 rounded-2xl font-black text-gray-600 hover:border-primary transition-all shadow-sm"><Filter className="w-5 h-5" />تصفية</button>
                    <button className="bg-primary text-white p-3 rounded-2xl hover:bg-primary/95 transition-all shadow-lg shadow-primary/25"><Plus className="w-6 h-6" /></button>
                  </div>
                </div>
                  ...
                  <table className="w-full text-right">
                    <thead>
                      <tr className="bg-gray-50 text-gray-400 uppercase text-[10px] font-black tracking-widest italic border-b">
                        <th className="px-8 py-5">الدواء التجاري</th><th className="px-8 py-5">المادة الفعالة</th>
                        <th className="px-8 py-5 text-center">البدائل</th><th className="px-8 py-5">المخزون</th>
                        <th className="px-8 py-5">السعر</th><th className="px-8 py-5">العمليات</th>
                      </tr>
                    </thead>
                        ...
                        <tr key={drug.id} className="hover:bg-gray-50/80 transition-all group">
                          <td className="px-8 py-6">
                            <div className="flex items-center gap-4">
                              <div className={cn("w-12 h-12 rounded-2xl flex items-center justify-center border border-primary/10 shadow-sm",
                                drug.isNearingExpiry ? "bg-rose-50 text-rose-500" : "bg-primary/5 text-primary"
                              )}><Package className="w-6 h-6" /></div>
                              <div>
                          ...
                          <td className="px-8 py-6"><p className="font-bold text-gray-500 italic text-sm">{drug.scientificName}</p></td>
                          <td className="px-8 py-6 text-center">
                            <div className="flex flex-wrap justify-center gap-1.5 min-w-[150px]">
                              {drug.brandNames.map(brand => <span key={brand} className="text-[9px] font-black bg-gray-100/50 text-gray-400 px-2 py-0.5 rounded-lg border border-gray-100 uppercase italic">{brand}</span>)}
                            </div>
                          </td>
                          <td className="px-8 py-6">
                            <div className="flex flex-col gap-2">
                              ...
                              <div className="flex justify-between items-center w-32">
                                ...
                                <div className={cn("flex items-center gap-1", drug.isNearingExpiry ? "text-rose-500" : "text-gray-400")}>
                                  <Clock className="w-3 h-3" /><span className="text-[9px] font-bold">{drug.expiry}</span>
                                </div>
                              </div>
                            </div>
                          </td>
                          ...
                          <td className="px-8 py-6">
                            <div className="flex gap-2">
                              <button onClick={() => { setActiveTab('dispensing'); setSelectedDrug(drug); }} className="px-4 py-2 bg-primary/10 text-primary rounded-xl font-bold text-xs hover:bg-primary hover:text-white transition-all shadow-sm">صرف</button>
                              <button className="p-2 bg-white border border-gray-100 text-gray-400 hover:text-primary rounded-xl shadow-sm transition-all"><MoreVertical className="w-5 h-5" /></button>
                            </div>
                          </td>
                        </tr>
                      ))}
            ...
            {activeTab === 'dispensing' && (
              <motion.div key="dispensing" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
                className="p-12 text-center space-y-6 flex flex-col items-center"
              >
                <div className="w-24 h-24 bg-primary/5 rounded-[2.5rem] flex items-center justify-center border-2 border-dashed border-primary/20"><Tablets className="w-10 h-10 text-primary" /></div>
                <div className="max-w-md">
                  <h3 className="text-2xl font-black text-gray-900">{selectedDrug ? `صرف ${selectedDrug.name}` : 'صرف وصفة طبية'}</h3>
                  <p className="text-gray-500 mt-2 font-medium">أدخل اسم المريض لتأكيد صرف الدواء.</p>
                </div>
                <div className="w-full max-w-lg space-y-4">
                  <input type="text" placeholder="اسم المريض أو رقم الهوية..." value={dispenseTarget} onChange={(e) => setDispenseTarget(e.target.value)}
                ...
                <div className="mt-8 pt-8 border-t w-full max-w-2xl">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {[{ name: 'سناء علي عبد الله', file: 'P-1001' }, { name: 'محمد حسن صالح', file: 'P-1002' }].map(p => (
                      <div key={p.file} className="p-4 bg-white border border-gray-100 rounded-2xl text-right flex items-center justify-between group hover:border-primary/20 transition-all cursor-pointer" onClick={() => setDispenseTarget(p.name)}>
                        <div><p className="font-bold text-gray-900">{p.name}</p><p className="text-[9px] text-gray-400 uppercase">File: {p.file}</p></div>
