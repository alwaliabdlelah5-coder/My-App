...
import { Calendar as CalendarIcon, Clock, User, Plus, ChevronRight, Filter, MoreVertical, CheckCircle2, XCircle } from 'lucide-react';
...
export default function AppointmentsPage() {
  ...
  const displayAppointments = loading ? initialAppointments : (liveApps.length > 0 ? liveApps.map(a => ({
    id: a.id, patient: a.patientName, doctor: a.doctorName, service: a.type,
    time: a.startTime, status: a.status, type: a.type
  })) : initialAppointments);
            ...
            <div className="bg-white rounded-[2.5rem] border shadow-sm p-8">
              ...
              <div className="grid grid-cols-7 gap-2">
                {Array.from({ length: 31 }).map((_, i) => (
                  <button key={i} className={cn("aspect-square rounded-xl flex items-center justify-center font-bold text-sm transition-all",
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-[2.5rem] border shadow-sm overflow-hidden min-h-[600px] flex flex-col">
              <div className="p-8 border-b bg-gray-50/50 flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <button className="flex items-center gap-2 px-6 py-2 border rounded-xl font-bold text-gray-500 hover:bg-gray-50 transition-all"><Filter className="w-4 h-4" />تصفية</button>
              </div>
                ...
                {displayAppointments.map((app, i) => (
                  ...
                  >
                    <div className="flex items-center gap-8">
                      ...
                      <div>
                        <div className="flex items-center gap-3">
                          ...
                          <span className={cn("px-2 py-0.5 rounded-md text-[8px] font-black uppercase italic tracking-widest",
                            app.type === 'urgent' ? "bg-rose-500 text-white" : app.type === 'new' ? "bg-emerald-100 text-emerald-700" : "bg-blue-100 text-blue-700"
                          )}>{app.type}</span>
                        </div>
                    ...
                    <div className="flex items-center gap-6">
                      <span className={cn("px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest italic flex items-center gap-2",
                        app.status === 'confirmed' ? "bg-emerald-50 text-emerald-500" : app.status === 'waiting' ? "bg-amber-50 text-amber-500" : "bg-gray-100 text-gray-400"
                      )}>
