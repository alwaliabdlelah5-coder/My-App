...
import { Users, Clock, ArrowUp, CheckCircle2, Play, Timer, User, MoreVertical, Activity } from 'lucide-react';
...
function QueueItemCard({ item, active, completed }: { item: any, active?: boolean, completed?: boolean }) {
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
      className={cn("p-6 rounded-3xl border-2 transition-all flex flex-col gap-4 cursor-pointer",
    ...
    >
      ...
      <div className="pt-4 border-t border-dashed flex items-center justify-between">
        <div className="flex items-center gap-2"><Clock className="w-3 h-3 text-gray-300" /><span className="text-[10px] font-black text-gray-400 italic">{item.waitingTime} in queue</span></div>
        {active ? (
          <div className="flex items-center gap-2"><Activity className="w-3 h-3 text-blue-500 animate-pulse" /><span className="text-[10px] font-black text-blue-500 uppercase tracking-widest italic">Live Consult</span></div>
        ) : (
...
export default function QueuePage() {
  ...
  const displayQueue: any[] = loading ? initialQueue : (liveQueue.length > 0 ? liveQueue.map(q => ({
    ...
    status: q.status, priority: q.priority, waitingTime: 'Real-time'
  })) : initialQueue);

  const filteredQueue = activeDoctor === 'All' ? displayQueue : displayQueue.filter(q => q.doctor === activeDoctor);

  return (
    <Sidebar>
      <div className="space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <StatCard label="المرضى في الانتظار" value={String(displayQueue.filter(q => q.status === 'waiting').length)} icon={Users} color="text-amber-500" bg="bg-amber-50" />
          <StatCard label="قيد الكشف" value={String(displayQueue.filter(q => q.status === 'in_progress').length)} icon={Play} color="text-blue-500" bg="bg-blue-50" />
          <StatCard label="المتوسط الزمني" value="18 min" icon={Timer} color="text-emerald-500" bg="bg-emerald-50" />
          <StatCard label="إجمالي من تم فحصهم" value={String(displayQueue.filter(q => q.status === 'completed').length)} icon={CheckCircle2} color="text-primary" bg="bg-primary/5" />
        </div>
          ...
          <div className="p-8 border-b bg-gray-50/50 flex flex-col lg:flex-row justify-between items-center gap-6">
            ...
            <div className="flex gap-2 p-1 bg-white border rounded-2xl shadow-inner">
              {['All', 'د. سارة خالد', 'د. علي يحيى', 'د. أحمد المحمدي'].map(doc => (
                <button key={doc} onClick={() => setActiveDoctor(doc)} className={cn("px-6 py-2.5 rounded-xl text-xs font-black transition-all",
          ...
          <div className="flex-1 p-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="space-y-6">
              <div className="flex items-center justify-between px-2">
                <h3 className="font-black text-xs uppercase tracking-widest text-amber-500 italic">Waiting Room ({filteredQueue.filter(q => q.status === 'waiting').length})</h3>
                <div className="w-8 h-1 bg-amber-200 rounded-full" />
              </div>
              <div className="space-y-4">{filteredQueue.filter(q => q.status === 'waiting').map(item => <QueueItemCard key={item.id} item={item} />)}</div>
            </div>
            <div className="space-y-6">
              <div className="flex items-center justify-between px-2">
                <h3 className="font-black text-xs uppercase tracking-widest text-blue-500 italic">In Consultations ({filteredQueue.filter(q => q.status === 'in_progress').length})</h3>
                <div className="w-8 h-1 bg-blue-200 rounded-full" />
              </div>
              <div className="space-y-4">{filteredQueue.filter(q => q.status === 'in_progress').map(item => <QueueItemCard key={item.id} item={item} active />)}</div>
            </div>
            <div className="space-y-6">
              <div className="flex items-center justify-between px-2">
                <h3 className="font-black text-xs uppercase tracking-widest text-emerald-500 italic">Completed ({filteredQueue.filter(q => q.status === 'completed').length})</h3>
                <div className="w-8 h-1 bg-emerald-200 rounded-full" />
              </div>
              <div className="space-y-4">{filteredQueue.filter(q => q.status === 'completed').map(item => <QueueItemCard key={item.id} item={item} completed />)}</div>
            </div>
          </div>
        </div>
      </div>
