...
import { Stethoscope, Search, Heart, Thermometer, Activity, Pill, FlaskConical, Save, Plus, Trash2 } from 'lucide-react';
    ...
    <Sidebar>
      <div className="space-y-8 h-full flex flex-col">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => { if (!selectedPatient) { alert('الرجاء اختيار مريض أولاً'); return; } alert(`تم حفظ السجل الطبي للمريض ${selectedPatient.name} بنجاح.`); }}
            ...
            <Save className="w-5 h-5" />حفظ السجل
          </button>
        </div>
          ...
          <div className="space-y-6 sticky top-28">
            <div className="bg-white rounded-3xl border shadow-sm overflow-hidden p-6 space-y-6">
              <div className="relative">
                ...
                <input type="text" placeholder="ابحث عن مريض..." className="w-full bg-gray-50 border-none rounded-2xl py-3 pr-11 pl-4 text-sm focus:ring-2 focus:ring-primary/20 transition-all" />
              </div>
              {!selectedPatient ? (
                <div className="space-y-2">
                  <p className="text-xs font-bold text-gray-400 px-2 uppercase tracking-wider">مرضى اليوم المجدولون</p>
                  {mockPatients.map(p => (
                    <button key={p.id} onClick={() => setSelectedPatient(p)} className={cn("w-full flex items-center gap-4 p-4 hover:bg-gray-50 rounded-2xl transition-all text-right border border-transparent",
                      ...
                      <div><p className="font-bold text-gray-900">{p.name}</p><p className="text-xs text-gray-500">{p.fileNumber}</p></div>
                    </button>
                  ))}
                </div>
              ) : (
                <div className="space-y-6">
                  ...
                  <button onClick={() => setSelectedPatient(null)} className="w-full py-2 text-xs font-bold text-rose-500 hover:underline border-t">تغيير المريض</button>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white rounded-3xl border shadow-sm min-h-[600px] flex flex-col overflow-hidden">
            ...
            <div className="p-8 flex-1">
              {activeSection === 'vitals' && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <VitalsInput label="ضغط الدم (انقباضي)" icon={Activity} unit="mmHg" placeholder="120" value={vitals.systolic || ''} onChange={(v) => setVitals(prev => ({ ...prev, systolic: v }))} />
                  <VitalsInput label="ضغط الدم (انبساطي)" icon={Activity} unit="mmHg" placeholder="80" value={vitals.diastolic || ''} onChange={(v) => setVitals(prev => ({ ...prev, diastolic: v }))} />
                  <VitalsInput label="النبض" icon={Heart} unit="bpm" placeholder="72" value={vitals.pulse || ''} onChange={(v) => setVitals(prev => ({ ...prev, pulse: v }))} />
                  <VitalsInput label="درجة الحرارة" icon={Thermometer} unit="°C" placeholder="37.0" value={vitals.temp || ''} onChange={(v) => setVitals(prev => ({ ...prev, temp: v }))} />
                </div>
              )}
              {activeSection === 'complaint' && (
                <div className="space-y-6">
                  <div className="space-y-2">
                    ...
                    <textarea rows={10} value={medicalHistory} onChange={(e) => setMedicalHistory(e.target.value)}
                  ...
                  <div className="flex gap-4 flex-wrap">
                    {['صداع', 'حمى', 'ألم مفاصل', 'غثيان', 'تعب عام'].map(tag => (
                      <button key={tag} onClick={() => setMedicalHistory(prev => prev + ' ' + tag)} className="px-4 py-2 bg-primary/10 text-primary rounded-xl text-xs font-bold hover:bg-primary/20">{tag}</button>
                    ))}
                  </div>
                </div>
              )}
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea rows={10} value={diagnosis} onChange={(e) => setDiagnosis(e.target.value)}
                      className="w-full bg-gray-50 border-none rounded-3xl p-6 text-sm focus:ring-2 focus:ring-primary/20" placeholder="نص حر للتشخيص والخطة..." />
                  </div>
                </div>
              )}
              {activeSection === 'prescription' && (
                <div className="space-y-8">
                  <div className="flex items-center justify-between">
                    <div><h3 className="font-black text-xl text-gray-900 italic tracking-tighter">Drug Prescription</h3><p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">مرتبط بقاعدة بيانات الصيدلية</p></div>
                    <button className="flex items-center gap-2 text-primary text-sm font-black hover:scale-105 transition-all"><Plus className="w-4 h-4" />إضافة دواء للوصفة</button>
                  </div>
                  <div className="p-8 bg-gray-50 rounded-[2rem] flex items-center justify-between shadow-sm">
                    <div className="flex items-center gap-6">
                      <div className="w-14 h-14 bg-white rounded-2xl flex items-center justify-center text-primary shadow-md border border-gray-100"><Pill className="w-7 h-7" /></div>
                      <div>
                    ...
                    <button className="p-3 bg-white text-rose-500 hover:bg-rose-500 hover:text-white rounded-xl transition-all shadow-sm"><Trash2 className="w-5 h-5" /></button>
                  </div>
                </div>
              )}
              {activeSection === 'labs' && (
                <div className="space-y-8">
                  <div className="flex items-center justify-between">
                    <div><h3 className="font-black text-xl text-gray-900 italic tracking-tighter">Lab & Radiology Orders</h3><p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">إرسال طلبات التحاليل والأشعة</p></div>
                    <button onClick={() => alert('إرسال طلب التحليل للمختبر بنجاح')} className="bg-gray-900 text-white px-6 py-2 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-primary transition-all shadow-lg">طلب فحص جديد</button>
                  </div>
            ...
            <div className="p-8 border-t bg-gray-50/50 flex justify-between items-center px-12">
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs font-black text-gray-400 uppercase tracking-widest italic">Autosaved</span>
              </div>
              <div className="flex items-center gap-4">
                <button className="text-gray-400 font-black text-xs uppercase tracking-widest hover:text-gray-900">حفظ كمسودة</button>
                <button onClick={() => alert('تم إصدار السجل الطبي بنجاح')} className="bg-primary text-white px-12 py-4 rounded-2xl font-black italic tracking-tighter text-lg shadow-xl shadow-primary/30 hover:scale-105 active:scale-95 transition-all">إصدار السجل الطبي</button>
              </div>
            </div>
          </div>
        </div>
